...
- **이 폴더에 테스트 파일(`*.unit.test.ts(x)`, `*.component.test.ts(x)`, `*.integration.test.ts(x)`)을 두지 않는다** — Unit/Component는 대상 파일 옆에 colocate하고 Integration은 `test/integration/`에 둔다(`docs/__test/README.md`). 이 폴더는 지원 자산 전용이다.
- **`setup/` 하위는 `vitest.config.ts`에서만 참조하고 배럴(`index.ts`)에 넣지 않는다** — `jsdom-polyfill.ts`는 import되는 즉시 전역(`Element.prototype`, `globalThis.ResizeObserver` 등)을 변조하므로, 배럴에 실리면 `@test/support`를 import하는 것만으로 node 환경 테스트까지 이 부수효과를 받는다.
