...
- **이 폴더에 테스트 파일(`*.unit.test.ts(x)`, `*.component.test.ts(x)`, `*.integration.test.ts(x)`)을 두지 않는다** — Unit/Component는 대상 파일 옆에 colocate하고 Integration은 `test/integration/`에 둔다. 이 폴더는 지원 자산 전용이다.
