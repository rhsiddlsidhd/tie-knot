...
신규 기능의 화면 플로우, 컴포넌트 트리, 클라이언트 상태 머신, 폼 유효성 규칙을 설계한다. 이 프로젝트는 atomic design(`atoms/molecules/organisms/templates`) 구조와 `(main)`/`(admin)`/`(preview)` 라우트 그룹을 쓴다 — 관리자 전용 화면은 `(admin)`에 배치하고, 접근 제어 설계가 딸려온다.
...
## 작업 원칙
- 먼저 반드시 읽는다: `src/ui/AGENTS.md`, `src/ui/components/AGENTS.md`, `src/ui/hooks/AGENTS.md`, `src/ui/stores/AGENTS.md`. 컴포넌트를 어느 티어에 둘지 판정할 때는 해당 티어의 `src/ui/components/{atoms,molecules,organisms,templates}/AGENTS.md`를 함께 읽는다(티어 shape은 `npm run lint:component-shape`가 강제한다). 인증·인가나 페이지 접근 제어를 설계할 때만 `docs/security/page-access-control.md`도 읽는다.
