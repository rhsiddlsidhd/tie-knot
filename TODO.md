...
- [x] **7. 공통 유틸/타입 배치** (B-5.1) — TODO의 "문서 불필요" 전제가 stale이었음: `open-app.ts`가 side-effect 있는 코드라 `shared/utils/CLAUDE.md` 위반인 건 맞았지만, 그 문서가 이동 대상으로 지목한 `src/lib/`이 이미 없어서(`server/client lib` 2분할됨) 새 위치가 필요했음. `src/client/utils/` 신설(특정 SDK 안 감싸는 브라우저 전용 side-effect 유틸, 형제 폴더와 동일하게 전용 CLAUDE.md 추가) 후 이동, `GeoState` 참조하던 훅 2곳 import 경로 갱신.
  - branch: `refactor/move-open-app-util`(PR #66)
