# CLAUDE.md — src/client/components/atoms/
...
molecule과의 경계는 "누가 조합했는가"다(`src/client/components/CLAUDE.md` 핵심 원칙 2) — shadcn/Radix 산출물은 조합 주체가 외부 라이브러리라 "물리적으로 더 못 쪼갠다"가 기준이 아니어도 atom 자격이 있고, 커스텀 프리미티브는 조합이 하나라도 있으면 atom 자격이 없다(molecules 소관).
