...
| File         | Purpose                                                                                                      |
| ------------ | ------------------------------------------------------------------------------------------------------------ |
| `fetcher.ts` | `useSWR` 전용 — `src/server/boundary.ts` route 응답 envelope 파싱, 실패 시 `ErrorPayload`로 정규화해서 throw |

## Structure

```
src/client/
├── components/   # atoms/molecules/organisms/templates — src/client/components/AGENTS.md
├── hooks/        # 프로젝트 자체 로직 커스텀 훅(외부 SDK 결합 훅 제외) — src/client/hooks/AGENTS.md
├── store/        # 전역 클라이언트 상태(Zustand) — src/client/store/AGENTS.md
├── context/      # 특정 도메인/UI 트리 한정 React Context 상태 — src/client/context/AGENTS.md
├── lib/          # 브라우저 전용 외부 연동(kakao/cloudinary) + cn — src/client/lib/AGENTS.md
├── utils/        # 브라우저 전용 side-effect 유틸, 특정 외부 SDK는 안 감쌈(감싸면 lib/) — src/client/utils/AGENTS.md
└── fetcher.ts    # Key Files 참고.
```

## Critical Convention

- **Server Action은 `fetcher` 계약 대상이 아니다.** Server Action의 실패는 `fetcher`/`boundary.ts` envelope를 거치지 않고 `useActionState` state(`ErrorPayload`)로 직접 내려온다 — `fetcher`를 Server Action 호출에 쓰지 않는다(`ErrorPayload` 타입 자체는 `src/shared/types` 공유 계약이다).
...
- 서비스 레이어가 던지는 에러 타입(`src/shared/types/error.ts`)은 이 경계 + 서비스 레이어 전용이다 — 서비스 함수가 이미 이 타입을 던지는 경우 Server Action이 받아서 리턴값으로 번역한다(`src/server/actions/AGENTS.md` Gotchas 참고).
...
| `AGENTS.md`         | `src/shared/types/`   | 응답/에러 타입 원본 확인 시                    | `types/error.ts`         |
