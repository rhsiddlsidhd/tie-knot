...
`store/`는 앱 전체 범위 클라이언트 상태를 모아둔다 — 두 성격이 있다: 도메인 데이터 상태(`auth.store.ts`, `order.store.ts`)와 모달 UI 상태(`guestbook.modal.store.ts`, `admin.modal.store.ts`). 특정 UI 트리에만 한정된 상태는 여기가 아니라 `src/context/`(`src/context/CLAUDE.md` 참고).
