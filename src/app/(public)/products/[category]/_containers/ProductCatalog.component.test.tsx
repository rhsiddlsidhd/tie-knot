...
vi.mock(
  "@/app/(public)/products/[category]/_components/ProductCatalog",
  () => ({
    ProductCatalog: ({
      products,
      category,
...
describe("ProductCatalog (container)", () => {
  ...
  it("firstPage를 fallbackData로 useSWRInfinite에 넘기고, 누적된 모든 페이지의 items를 View에 전달한다", () => {
    ...
    expect(screen.getByTestId("available-sub-categories")).toHaveTextContent(
      "wedding",
    );
  ...
  it("SWR이 여러 페이지를 반환하면 items를 flatMap으로 누적한다", () => {
    useSWRInfiniteMock.mockReturnValue({
      data: [
        buildPage({
          items: [buildProduct({ title: "1페이지" })],
          nextCursor: "cursor-2",
        }),
        buildPage({
          items: [buildProduct({ title: "2페이지" })],
          nextCursor: null,
        }),
      ],
      size: 2,
  ...
  it("getKey는 첫 페이지에서 category/subCategory 쿼리를 담은 key를 만든다", () => {
    ...
    expect(getKey(0, null)).toBe(
      "/api/products?category=favor&subCategory=candle",
    );
  });
