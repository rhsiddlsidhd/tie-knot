...
const MobileInvitationStatusControls = ({ orderId, status }: { orderId: string; status?: "draft" | "published" }) => {
  ...
  return (
    <div className="mb-6 flex flex-wrap items-center gap-3">
      ...
      <Button variant="outline" asChild><Link href={routes.myOrders.mobileInvitationPreview(orderId)}>미리보기</Link></Button>
      {currentStatus !== "published" && <p className="text-warning text-sm">계좌 정보가 비어 있어도 발행할 수 있습니다. 공개 전 내용을 확인해 주세요.</p>}
    </div>
  );
}
