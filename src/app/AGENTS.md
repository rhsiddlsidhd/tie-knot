...
- **Provider 소멸**: root layout 자체엔 provider가 없지만 `각 Route Group layout.tsx`의 Provider/Toaster까지 같이 사라진다 — 렌더하는 컴포넌트는 provider 없이도 동작하는 self-contained 컴포넌트여야 한다(예: `organisms/ErrorFallback.tsx` — atoms만 조합, context 의존 없음).
...
라우트 그룹은 공유 `layout.tsx` 또는 독립 `error.tsx` 근거가 있을 때만 만든다 — 조직 편의만으로 만들지 않는다. 현재 Route Group은 `(public)`, `(auth)`, `(account)`, `(checkout)`, `(admin)`, `(preview)`로 나누며 각 그룹은 URL에 드러나지 않는 레이아웃·오류 경계를 소유한다.

## Structure

- 라우트 전용 부속물(타입/순수함수/상수/서브 UI/컨테이너/훅)은 Next 공식 private 폴더(`_folder`)로 분리한다 — **필요한 것만** 생성, 빈 폴더 강제 금지. 목표 형태(`(checkout)/payment/` 기준):
  ```
  (checkout)/payment/
...
- 위 조건 중 Server Action 결합은 ESLint가 강제한다 — `_components/` 아래 제품 파일이 `@/actions/*`를 값으로 import하면 `@typescript-eslint/no-restricted-imports`가 막는다(타입 전용 import와 테스트는 제외). SWR 페칭과 zustand 파생 side effect는 정적으로 판정할 수 없어 규칙 대상이 아니며 리뷰가 잡는다. `layout.tsx` 셸 조각은 도메인 로직을 가져도 되므로(아래 Critical Conventions) 예외가 필요하면 사유를 적은 `eslint-disable-next-line`을 그 줄에 붙인다 — `Route Group의 _components/`는 그룹 셸과 홈 라우트를 겸해 폴더 단위로 분리할 수 없어서다.
