...
- Server Action 파일에 `import "server-only"`를 추가하지 않는다 — Client Component가 `"use server"` export를 RPC 참조로 import하는 정상 경로를 차단할 수 있다. 이 폴더에는 `"use server"`가 선언된 Action만 두고, 클라이언트에 노출하면 안 되는 일반 서버 helper는 해당 역할의 서버 계층에 둔다.
...
- `models/`, `db/`, `adapters/`를 직접 import하지 않는다. 특히 `adapters/server/`도 직접 import하지 않는다. 인증·외부 SDK·보상 로직과 업무 흐름 조합은 `services/`의 유스케이스 함수가 담당한다.
