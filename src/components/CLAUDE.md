...
- **atoms/molecules/organisms 3단계.** Brad Frost의 Atomic Design 원본 5단계(atoms/molecules/organisms/templates/pages) 중 이름 3개만 차용했다 — templates/pages 역할은 `src/app/**/layout.tsx`/`page.tsx`가 대신한다(`src/app/CLAUDE.md` 참고).
- **두 개의 독립된 축으로 구분한다 — 헷갈리면 이 구분으로 돌아온다.**
  - **축 A — 티어(atom/molecule/organism) 분류.** 원본 Atomic Design 기준 그대로 쓴다: 조합 복잡도/책임 범위로 판단한다(각 티어 경계는 `src/components/atoms/CLAUDE.md`/`molecules/CLAUDE.md`/`organisms/CLAUDE.md` 참고). "몇 곳에서 재사용되는가"는 이 축과 무관하다.
  - **축 B — 물리적 위치(공용 `src/components/{tier}/` vs 라우트 전용 `_components/`).** 원본에 없는, 이 프로젝트가 독자적으로 추가한 것 — 순수성 원칙(핵심 원칙 1)과 소비자 수(`src/app/CLAUDE.md` 컨테이너 승격 규칙)로 판단한다. 어떤 티어든 축 B는 동일하게 적용된다.
  - 과거 `fields/`·`(preview)/` 서브폴더를 "예외존"처럼 다룬 게 혼란의 원인이었다 — 서브폴더에 있다는 사실 자체는 두 축 어느 쪽 판단에도 영향을 주지 않는다.
- **핵심 원칙 1 — atoms/molecules/organisms 전부 순수(presentational)하다(축 B 관련).** 도메인 로직·데이터 페칭·Server Actions을 이 3단계 어디에도 두지 않는다 — props로만 데이터/핸들러를 받는다. 도메인 로직이 필요한 화면은 그 로직을 감싸는 **컨테이너**(`src/app/**/_components/{Name}.tsx`, 라우트 전용)가 순수 organism/molecule을 import해서 props를 채워 넣는 방식으로 만든다.
  ...
  - 소비자 1곳이 `page.tsx`가 아니라 `layout.tsx`(라우트 그룹 전체를 감싸는 셸)여도 동일하게 적용한다 — 예: `Header`/`AuthButtons`/`UserAccountNav`/`Footer`는 소비처가 `(main)/layout.tsx`·`page.tsx` 하나뿐이라 `(main)/_components/`로, `GuestbookModal`은 `(preview)/layout.tsx` 하나뿐이라 `(preview)/_components/`로 옮겼다(`src/components/layout/`은 이 정리로 폐기 — atoms/molecules/organisms 3단계 밖의 미분류 폴더였다).
