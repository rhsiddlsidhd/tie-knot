...
- `.lean()` 결과의 ObjectId 필드는 services에서 명시적으로 `.toString()` 변환한다 — 모델 `toJSON` transform에 기대지 않는다(`src/server/models/AGENTS.md` 참고).
...
| 문서                 | 위치                   | 트리거                              | 요약                               |
| -------------------- | ----------------------- | ------------------------------------ | ----------------------------------- |
| `AGENTS.md`          | `src/server/models/`    | DB 스키마 확인 시                    | 모델 정의                           |
