...
- `utils/error.ts`의 `handleClientError`는 `fetcher`/`apiRequest`가 throw한 에러를 받아 필드에러/메시지/void 중 뭘 보여줄지 판단하는 UI 로직이다(`src/api/CLAUDE.md` 참고). `console.error` 호출이 있어 "side-effect 없음" 원칙과 살짝 어긋나 보일 수 있는데, 진단용 로깅은 이 프로젝트에서 순수성 위반으로 안 친다(다른 service/util 파일들도 동일하게 취급).
