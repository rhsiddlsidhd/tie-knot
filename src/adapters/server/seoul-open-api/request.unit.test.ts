...
describe("fetchSeoulOpenApi", () => {
  ...
  it("정상 JSON 응답을 rows 배열로 리턴한다", async () => {
    ...
    vi.mocked(fetch).mockResolvedValueOnce(
      mockResponse(200, true, "application/json", body),
    );
  ...
  it("ok:true이지만 XML 본문(인증키 오류 등)이면 EXTERNAL_SERVICE로 분류한다", async () => {
    const xml =
      "<RESULT><CODE>INFO-100</CODE><MESSAGE>인증키가 유효하지 않습니다.</MESSAGE></RESULT>";
    vi.mocked(fetch).mockResolvedValueOnce(
      mockResponse(200, true, "text/xml", xml),
    );

    await expect(
      fetchSeoulOpenApi<Row>(SERVICE_NAME, [1, 1000]),
  ...
  it("ok:false(HTTP 5xx)면 EXTERNAL_SERVICE로 분류하고 message에 status를 남긴다", async () => {
    vi.mocked(fetch).mockResolvedValueOnce(
      mockResponse(500, false, "text/html", "<html>error</html>"),
    );
  ...
  it("content-type은 JSON인데 본문이 깨져 있으면 EXTERNAL_SERVICE로 분류한다", async () => {
    vi.mocked(fetch).mockResolvedValueOnce(
      mockResponse(200, true, "application/json", ""),
    );
  ...
  it("결과 0건(INFO-200)은 실패가 아니라 빈 배열이다", async () => {
    ...
    const rows = await fetchSeoulOpenApi<Row>(SERVICE_NAME, [
      1,
      50,
      "존재안함역",
    ]);
