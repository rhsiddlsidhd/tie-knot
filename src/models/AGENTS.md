...
`models/`는 Mongoose 스키마 정의 + 그 스키마가 만드는 Document/JSON 타입 + 컴파일된 Model을 모아둔다 — 파일 하나에 셋(스키마/타입/모델)이 다 들어간다. API/도메인 계약 타입(`src/core/types/`)과는 소유권이 다르다: 여기는 "DB에 실제로 뭐가 저장되는가", `types/`는 "API가 뭘 주고받는가".
...
- 이 모델을 조회/조작하는 비즈니스 로직: `src/services/AGENTS.md`
