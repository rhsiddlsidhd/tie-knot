...
- 외부 SDK를 초기화하는 훅(`useKakaoLoader` 등)을 여기 두지 않는다 — 훅이라도 연동 대상 하나에 강결합돼 있으면 `src/ui/lib/{service}/`의 "연동 대상 1개 = 폴더 1개" 소관이다(`src/ui/lib/AGENTS.md` 참고).
...
- 외부 SDK 초기화 훅의 실제 위치(lib과의 배치 경계): `src/ui/lib/AGENTS.md`
