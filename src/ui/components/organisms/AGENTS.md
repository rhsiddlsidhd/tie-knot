...
- 소비자가 라우트 1곳뿐인 organism을 미리 여기로 승격하지 않는다 — 도메인 결합 여부와 무관하게, 순수한 완성품 자체가 2개 이상의 라우트/컨테이너에서 재사용될 때만 여기 둔다(`src/client/components/AGENTS.md` 컨테이너 승격 규칙). 유일한 소비자가 라우트가 아니라 이미 공유 중인 다른 organism/molecule이면 이 규칙 대상이 아니다(같은 예외는 `src/client/components/AGENTS.md` 참고).
...
| `AGENTS.md`            | `src/client/components/molecules/`   | 조합 재료 확인 시                   | molecule 정의                 |
