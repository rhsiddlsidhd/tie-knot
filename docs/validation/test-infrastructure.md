...
- `unit`: actions, core, 서버/순수 adapter, API 경계의 `*.test.ts`를 Node에서 실행한다.
- `component`: ui, app UI, 브라우저 adapter의 `*.test.ts(x)`를 jsdom에서 실행한다.
- `integration`: actions/db/services/API와 DB를 관통하는 app integration을 실행한다. 단일 replica set을 공유하므로 파일을 직렬 실행한다. jsdom이 필요한 파일은 `@vitest-environment jsdom`을 선언한다.
- `integration-component`: MongoDB 없이 Hook·컴포넌트 경계를 관통하는 `*.integration.test.tsx`를 jsdom에서 실행한다.
...
- `npm run test:component`: component와 DB 없는 jsdom integration
