...
테스트는 사용하는 라이브러리가 아니라 실제로 통과하는 경계로 분류한다. 공개 taxonomy는 unit, integration, E2E 세 가지다.
...
`integration-client`, `integration-server`, `integration-app`은 공개 분류가 아니라 실행 환경과 공유 자원 제약을 처리하는 Vitest project다. 경로에 따라 project가 선택되므로 현재 include/exclude는 `vitest.config.ts`를 확인한다.
