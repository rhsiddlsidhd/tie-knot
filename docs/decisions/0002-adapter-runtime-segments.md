...
대신 `src/` 안에서 Adapter만 역할 아래에 런타임 축을 갖는다. 런타임을 디렉터리에 중복하지 않기로 한 ADR-0001의 원칙에 예외가 생기므로 새 경계를 추가할 때 어떤 축을 먼저 둘지 한 번 더 판단해야 한다. 한 서비스가 두 런타임을 가지면 구현이 `server/`와 `browser/`로 나뉘어 서비스 단위 응집도는 낮아진다.
...
- 런타임 세그먼트 적용: [PR #61](https://github.com/rhsiddlsidhd/tie-knot/pull/61)
